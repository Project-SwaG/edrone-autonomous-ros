...
class Edrone():
    ...
    def __init__(self):
        ...
        self.Kp_v = [2.158, 2.158, 900]  # 650]
        self.Ki_v = [0, 0, 0]
        self.Kd_v = [156, 156, 500]  # 25]
        ...
        filepath = rospack.get_path('vitarana_drone')+'/scripts/original.csv'
...
def avoid_obstacle_new(position, speed):
    ...
    flag2 = 0
    while((e_drone.laser_negative_latitude <= 4 and e_drone.laser_negative_latitude >= 0.5 or
           e_drone.laser_negative_longitude <= 4 and e_drone.laser_negative_longitude >= 0.5 or
           e_drone.laser_positive_latitude <= 4 and e_drone.laser_positive_latitude >= 0.5 or
           e_drone.laser_positive_longitude <= 4 and e_drone.laser_positive_longitude >= 0.5)):
        multiplier = 0.00022
        print("in obstacle")
        ...
        while(is_at_setpoint2D(e_drone.setpoint_final)):
            ...
            if(delivery or True):
                e_drone.setpoint_location[2] = max(
                    (e_drone.setpoint_location[2] - 0.03), e_drone.setpoint_final[2] + 8)
...
def generate_csv():
    ...
    filepath = rospack.get_path('vitarana_drone') + \
        '/scripts/sequenced_manifest_original.csv'
